...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Boundary width in pixels (default: 5).",
    )
    parser.add_argument(
        "--tif-files",
        nargs="+",
...
def save_png(tif_path: Path, dpi: int) -> Path:
    with rasterio.open(tif_path) as src:
        ...
        if src.count == 3:
            ...
            if "coherence" in tif_path.name:
                coherence_scale = matplotlib.cm.ScalarMappable(
            ...
            if "coherence" in tif_path.name:
                colourbar = fig.colorbar(
                    plotted, ax=ax, shrink=0.8, label="Coherence"
                )
...
def process_tif(
    ...
    with rasterio.open(input_tif) as src:
        ...
        # Display coherence in grayscale beneath the red fast-ice boundary.
        # The fast-ice-only coherence product remains a single-band raster and
        # is plotted separately with the jet colour map in save_png().
        vmin, vmax, cmap = display_parameters(input_tif.name, data, valid)

        if "coherence" in input_tif.name:
            # Contrast stretch for the grayscale visualisation only
            coherence_values = data[valid]
            boundary_vmin, boundary_vmax = np.nanpercentile(
                coherence_values,
                [2, 98]
            )
            boundary_cmap = "gray"
        else:
            boundary_vmin, boundary_vmax = vmin, vmax
            boundary_cmap = cmap

        scaled = np.zeros(data.shape, dtype="float32")
        scaled[valid] = np.clip(
            (data[valid] - boundary_vmin)
            / (boundary_vmax - boundary_vmin),
            0.0,
            1.0,
        )

        rgb = (plt.get_cmap(boundary_cmap)(scaled)[..., :3] * 255).astype("uint8")
        rgb[~valid] = [0, 0, 0]
